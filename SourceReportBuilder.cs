...
internal static class SourceReportBuilder {
	...
	static List<RequestSourceFinding> BuildRequestSourceFindings(
		...
		foreach (var piece in BuildRequestPieces(targetSession)) {
			if (string.Equals(piece.PieceKind, "path", StringComparison.OrdinalIgnoreCase)) {
	...
	static List<RequestSourceFinding> BuildPathSourceFindings(
		...
		while (!string.IsNullOrWhiteSpace(remaining)) {
			...
			findings.Add(new RequestSourceFinding(pathPiece.PieceKind, pathPiece.Name, foundPath, source));
	...
	static RequestSourceFinding BuildFinding(
		...
		Dictionary<string, int>? missingIndexes) {
		if (source is not null)
			return new RequestSourceFinding(piece.PieceKind, piece.Name, piece.Value, source);
		...
		if (missing is not null && missingIndexes is not null) {
			...
			return new RequestSourceFinding(piece.PieceKind, piece.Name, piece.Value, new MissingSourceReference(missingIndex));
		}

		return new RequestSourceFinding(piece.PieceKind, piece.Name, piece.Value, new MissingSourceReference(-1));
	}
