...
app/Auth/
  ...
  ExchangeClassifier.cs           (rewrite of ClassifyRequest/ClassifyResponse/ClassifyUnknownExchanges/ClassifyExchange)
  AuthFlowType.cs
  ...
  AuthFlowDetector.cs            (public entry point: Detect(exchanges))
...
app/AzureB2c/                    (B2C enrichment only, from here on)
  ...
  B2cFlowDetails.cs
  B2cEnricher.cs                 (Enrich(DetectedAuthenticationFlow, exchanges) -> B2cFlowDetails?)
  # DELETE: AzureB2cAuthenticationScanner.cs, AuthenticationReport.cs, RequestType.cs, ResponseType.cs
```

`app/models/log/Request.cs` / `Response.cs` retype their classification fields from `AzureB2c.RequestType`/`AzureB2c.ResponseType` to `Auth.RequestType`/`Auth.ResponseType` (same enum member names, same `JsonIgnore(WhenWritingDefault)` behavior — `.exchanges.json` shape unaffected). `app/models/RequestSources.cs` same retype.
...
record OidcDiscoveryDocument(
    int SourceExchangeId, string? Issuer, string? AuthorizationEndpoint, string? TokenEndpoint,
...
record Variable(
    ...
    int ExchangeId, string? JsonPath, string? DerivedFromVariableName, string? Notes
);
...
record DetectedAuthenticationFlow(
    ...
    int? DiscoveryRequestExchangeId, int? AuthorizationRequestExchangeId,
    int? AuthorizationCallbackExchangeId, int? TokenRequestExchangeId,
...
record FlowWarning(FlowWarningKind Kind, string Message, IReadOnlyList<int> RelatedExchangeIds);
...
record AuthFlowDetectionResult(
    ...
    List<RequestClassification> ExchangeClassifications,  // flat per-exchange list, kept for debugging
...
Exchanges arrive already ordered by `ExchangeId` ascending (`SazPlanBuilder.Build`'s `OrderBy(kvp => kvp.Key)`) and already classified via `ExchangeClassifier.ClassifyUnknownExchanges`. Matching only looks backward in exchange order (mirrors the existing `priorExchanges` pattern).

1. Bucket classified exchanges by `RequestType`: `discovery[]`, `authRequests[]`, `callbacks[]`, `tokenRequests[]` (split by grant type), `deviceAuthRequests[]`.
2. Parse `OidcDiscoveryDocument` from every `discovery` exchange's response JSON.
...
6. Match `discovery -> flows`: issuer host equals flow's authorize/token endpoint host, else nearest preceding discovery exchange on same host; missing entirely → queue `MissingDiscoveryDocument` warning.
...
9. Any OAuth-shaped exchange never absorbed into a flow becomes its own partial flow with an appropriate warning (`IncompleteFlow`/`MissingCallback`/`MissingTokenExchange`) — every classified exchange ends up in exactly one flow. Exchanges that never matched any endpoint heuristic never enter flow-building (matches today's `Scan_ExcludesNonOauthB2cExchanges` behavior).

## Reuse vs. Rewrite of `AzureB2cAuthenticationScanner.cs`

**Reused near-verbatim** → `Auth/OAuthParameterHelpers.cs`: `TryGetRequestParameter`, `HasResponseType`, `TryParseFragmentParameters`, `HasCodeAndState`, `UrlsMatchIgnoringFragment`, `TryParseFragmentFromLocation`, `IsOauth2_AuthorizationRequest` (already generic — only checks `response_type`+`client_id` presence). The callback-detection-via-prior-authorize-exchange loop is already generic (calls `IsOauth2_AuthorizationRequest`, not a path check) — ports unchanged into `FlowCorrelator`/`ExchangeClassifier`.
...
- `Request.RequestType` / `Response.ResponseClassification` **stay** as before-the-fact per-exchange fields (just retyped to `Auth.*`) — they're persisted directly to `.exchanges.json` and consumed independently by `.sources.json`, which doesn't need cross-exchange flow grouping. The new `DetectedAuthenticationFlow` grouping is purely additive and only feeds `.b2c.json`.
- `app/Program.cs`: `AzureB2c.AzureB2cAuthenticationScanner.ClassifyUnknownExchanges` → `Auth.ExchangeClassifier.ClassifyUnknownExchanges` (same shape). `WriteAzureB2cReportAsJson` body now serializes `Auth.AuthFlowDetectionResult` from `Auth.AuthFlowDetector.Detect(classifiedExchanges)` instead of the old `AzureB2c.AuthenticationReport`; file path/extension (`.b2c.json`) unchanged.
- `app/SazPlanBuilder.cs` (`WriteAllExchangeSourcesReport`): swap to `Auth.ExchangeClassifier.ClassifyUnknownExchanges`. No shape change to `.sources.json`.
- `app/SourceReportBuilder.cs` (`BuildAzureB2cSourceContext`): swap `AzureB2cAuthenticationScanner.Scan(exchanges).Requests.Select(...)` for `Auth.AuthFlowDetector.Detect(exchanges).Flows.Where(f => f.IsAzureB2c).SelectMany(f => f.RelatedExchangeIds).ToHashSet()`, still computed once outside the per-exchange loop. `ClassifyExchange(targetExchange, previousExchanges)` → `Auth.ExchangeClassifier.ClassifyExchange(...)`.
- Method names `ClassifyUnknownExchanges`/`ClassifyExchange` are preserved (moved to `Auth.ExchangeClassifier`); `Scan` is effectively replaced by `Auth.AuthFlowDetector.Detect` since the return shape fundamentally changes.

## Test Strategy

- **`tests/AzureB2cAuthenticationScanner_Tests.cs`** → split:
  - Port the 10 pure classification tests into `tests/ExchangeClassifier_Tests.cs` (target `Auth.ExchangeClassifier`/`Auth.RequestType`/`Auth.ResponseType`). Add: a non-Azure provider case (`login.example.com/connect/authorize` + `/connect/token`, no `oauth2/v2.0` anywhere), and new grant-type cases for `client_credentials`, `password`, `device_code` token requests (currently untested/unclassified).
...
- **Shared `BuildExchange` helper**: currently duplicated across test files, missing request-header, fragment, and multi-Set-Cookie support. Extract to `tests/TestExchangeBuilder.cs`, add `requestHeaders` and `fragment` parameters (needed for Secret-header and implicit/fragment-mode tests); keep existing params. Note multi-`Set-Cookie` collapsing into one dict key as a pre-existing `Response.Headers` model limitation — not fixed in this pass.
...
1. **Endpoint classification**: `OAuthParameterHelpers.cs`, `EndpointClassifier.cs`, `OidcDiscoveryDocument.cs` + parser, `ExchangeClassifier.cs` (new grant-type branches). Port `ExchangeClassifier_Tests.cs`. Wire `Program.cs`/`SazPlanBuilder.cs`/`SourceReportBuilder.cs`. Checkpoint: solution compiles, `.exchanges.json`/`.sources.json` unchanged.
